...
const xml = fs.readFileSync('c:/Site-prime-products/premier_docx_extracted/word/document.xml', 'utf8');
const relsXml = fs.readFileSync('c:/Site-prime-products/premier_docx_extracted/word/_rels/document.xml.rels', 'utf8');
    ...
    if (match) {
      ...
      if (target) {
        output.push({
          ...
          contextText: currentText.trim().substring(currentText.trim().length - 100) // last 100 chars before image
        });
        currentText = ""; // reset
      }
    }
