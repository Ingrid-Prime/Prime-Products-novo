const renderImages = (images: string[], scaleClass: string = '') => {
  ...
  return `<div class="${containerClass}">` +
    images.map(img => `<img src="/images/tekno-valves/${img}" alt="Valve" class="w-28 h-28 object-contain mix-blend-multiply transition-transform duration-300 flex-shrink-0 ${scaleClass} hover:!scale-125" />`).join('') +
    `</div>`;
};
